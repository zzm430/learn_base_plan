...
class PolygonEnv:
    ...
    def step(self, action):
        ...
        if next_node in self.visited:
            return self.current_node, -10, False  # 重复访问，给予负奖励
    ...
    def render(self):
        ...
        # 绘制多边形
        plt.plot(np.append(nodes[:, 0], nodes[0, 0]), np.append(nodes[:, 1], nodes[0, 1]), 'r-', label='Polygon')
    ...
    def act(self, state, visited):
        if np.random.rand() <= self.epsilon:
            ...
            if available_actions:
                ...
                return random.randrange(self.action_size)  # 如果没有未访问的节点，随机选择
        state = torch.FloatTensor(state).unsqueeze(0)
        act_values = self.model(state)
        # 过滤掉已访问的节点
        act_values = act_values.squeeze(0).detach().numpy()  # 使用 detach() 分离张量
        for i in visited:
            act_values[i] = -np.inf  # 将已访问节点的Q值设为负无穷
        return np.argmax(act_values).item()
...
for e in range(episodes):
    ...
    while not done:
        action = agent.act(state, env.visited)  # 传入已访问的节点
